...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling the AddIngredient of the onion decorator without an IngredientAdd on its Ingredient field must return an error , not a string with '%s'", onionResult)
	}
...
func TestPizzaDecorator_FullStack(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When asking for a pizza with onion and meat the returned string must contain the text '%s' but '%s' din't have it", expectedText, pizzaResult)
	}
	t.Log(pizzaResult)
}
